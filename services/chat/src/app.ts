...
export function createApp() {
  ...
  app.use(
    pinoHttp({
      ...
      customProps: function customProps(req) {
        return {
          tenant: (req as any).tenantId || (req.body && typeof req.body === "object" ? (req.body as any).tenant_id : undefined),
        };
      },
  ...
  app.post("/chat", async (req, res) => {
    ...
    const { tenant_id, question, top_k = 5, use = "local", tenant_config } = req.body as any;
    ...
    const qVec = use === "gemini" ? await embedGemini(question) : (await embedLocal([question]))[0];
